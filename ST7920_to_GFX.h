 ...
 *
 *  1. replace "ST7920_SPI.h" with "ST7920_to_GFX.h"
 *  2. replace ST7920_SPI constructor with ST7920_to_GFX(lcd_cs)
 *  3. 
 *  4. ST7920_to_GFX uses FreeFonts or NULL (System 5x7)
 *  5. I have made no attempt to support cbm80amiga's font or text methods.
 *
 */

#warning   @@@@@@@@@@@@@@@@@@@@@@@@@ <ST7920_to_GFX.h> @@@@@@@@@@@@@@@@@@@@@@@@@@

#ifndef ST7920_to_GFX_H_
#define ST7920_to_GFX_H_
...
class ST7920_to_GFX : public ST7920
{
    public:
        ST7920_to_GFX(int CS) : ST7920(CS) { }
        void init(void) {
            ST7920::begin();
            setDither(8);
            setTextColor(BLACK);  //.kbv
        }
#if IMPL_KBV > 2
        void sendCmd(byte b);
        ...
        void drawLineH(uint8_t x0, uint8_t x1, uint8_t y, uint8_t col) {
            ST7920::fillRect(x0, y, x1 - x0 + 1, 1, col);
        }
        void drawLineV(uint8_t x,  uint8_t y0, uint8_t y1, uint8_t col) {
            ST7920::fillRect(x, y0, 1, y1 - y0 + 1, col);
        }
        ...
        int strWidth(char *txt) {
            int16_t pos, x1, y1;
            uint16_t len, w, h;
            bool is_gfx = (ST7920::gfxFont != NULL);
...
const byte ST7920_to_GFX::ditherTab[4 * 17] PROGMEM = {
